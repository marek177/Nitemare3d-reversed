...
const std::vector<Placement>& placements() {
    static const std::vector<Placement> p = {
...
{3,10,14,"Penelope",1,true},{3,10,15,"Dr. Hamerstein",1,true}
    };
    return p;
}
...
int64_t oneAwardPerPlacementBaseline(bool excludePenelopePenalty){
    int64_t t=0;
    for(auto&p:placements()){
        if(!p.playable)continue;
        uint8_t cls=(p.guard>=1&&p.guard<=24)?static_cast<uint8_t>(p.guard+7):0;
        if(excludePenelopePenalty&&p.guard==14)continue;
        t+=static_cast<int64_t>(scoreForClass(cls))*p.count;
    }
    return t;
}
} // namespace n3d::re
